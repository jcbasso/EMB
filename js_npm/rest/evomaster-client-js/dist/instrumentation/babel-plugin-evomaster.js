...
const InjectedFunctions_1 = __importDefault(require("./InjectedFunctions"));
const ObjectiveNaming_1 = __importDefault(require("./shared/ObjectiveNaming"));
const ref = "__EM__";
function evomasterPlugin(babel) {
    const t = babel.types;
    let statementCounter = 0;
    let branchCounter = 0;
    const objectives = Array();
    let fileName = "filename";
    function addBlockIfNeeded(path) {
    ...
    function replaceUnaryExpression(path) {
        ...
        const call = t.callExpression(t.memberExpression(t.identifier(ref), t.identifier(InjectedFunctions_1.default.not.name)), [exp.argument]);
        path.replaceWith(call);
    }
    function replaceLogicalExpression(path) {
      ...
      */
        ...
        const methodName = exp.operator === "&&" ? InjectedFunctions_1.default.and.name : InjectedFunctions_1.default.or.name;
        /*
        ...
        const call = t.callExpression(t.memberExpression(t.identifier(ref), t.identifier(methodName)), [left, right, t.booleanLiteral(pure),
            t.stringLiteral(fileName), t.numericLiteral(l), t.numericLiteral(branchCounter)]);
        objectives.push(ObjectiveNaming_1.default.branchObjectiveName(fileName, l, branchCounter, true));
        objectives.push(ObjectiveNaming_1.default.branchObjectiveName(fileName, l, branchCounter, false));
        path.replaceWith(call);
        branchCounter++;
    }
    /**
     ...
     */
    function doesContainAwaitExpression(node) {
    ...
    function replaceBinaryExpression(path) {
        ...
        */
        ...
        const l = exp.loc.start.line;
        const call = t.callExpression(t.memberExpression(t.identifier(ref), t.identifier(InjectedFunctions_1.default.cmp.name)), [exp.left, t.stringLiteral(exp.operator), exp.right,
            t.stringLiteral(fileName), t.numericLiteral(l), t.numericLiteral(branchCounter)]);
        objectives.push(ObjectiveNaming_1.default.branchObjectiveName(fileName, l, branchCounter, true));
        objectives.push(ObjectiveNaming_1.default.branchObjectiveName(fileName, l, branchCounter, false));
        path.replaceWith(call);
    ...
    function replaceConditionalExpression(path) {
        ...
        objectives.push(ObjectiveNaming_1.default.statementObjectiveName(fileName, l, statementCounter));
        exp.consequent = t.callExpression(t.memberExpression(t.identifier(ref), t.identifier(InjectedFunctions_1.default.ternary.name)), [consequent,
        ...
        statementCounter++;
        objectives.push(ObjectiveNaming_1.default.statementObjectiveName(fileName, l, statementCounter));
        exp.alternate = t.callExpression(t.memberExpression(t.identifier(ref), t.identifier(InjectedFunctions_1.default.ternary.name)), [alternate,
            t.stringLiteral(fileName), t.numericLiteral(l), t.numericLiteral(statementCounter)]);
        if (alternateIsAwait)
            exp.alternate = t.awaitExpression(exp.alternate);
        statementCounter++;
    }
    function replaceMemberExpression(path) {
        ...
        */
        ...
        if (path.parent && (t.isUpdateExpression(path.parent) ||
            ...
            (t.isAssignmentPattern(path.parent) && (path.parent.left == path.node))) ||
        ...
        const replaced = t.callExpression(t.memberExpression(t.identifier(ref), t.identifier(InjectedFunctions_1.default.squareBrackets.name)), [t.stringLiteral(fileName), t.numericLiteral(l), t.numericLiteral(branchCounter), obj,
    ...
    function replaceCallExpression(path) {
        ...
        */
        ...
        if (t.isMemberExpression(call.callee)) {
            replaced = t.callExpression(t.memberExpression(t.identifier(ref), t.identifier(InjectedFunctions_1.default.callTracked.name)), [t.stringLiteral(fileName), t.numericLiteral(l), t.numericLiteral(branchCounter),
                // @ts-ignore
                call.callee.object,
            ...
            replaced = t.callExpression(t.memberExpression(t.identifier(ref), t.identifier(InjectedFunctions_1.default.callBase.name)), [t.arrowFunctionExpression([], call, doesContainAwaitExpression(call))]);
            // @ts-ignore
            call.evomaster = true;
        }
        path.replaceWith(replaced);
    }
    function addLineProbeIfNeeded(path) {
        ...
        objectives.push(ObjectiveNaming_1.default.lineObjectiveName(fileName, l));
        objectives.push(ObjectiveNaming_1.default.statementObjectiveName(fileName, l, statementCounter));
        if ((t.isReturnStatement(stmt) && !stmt.argument)
            ...
            const mark = template_1.default.ast(`${ref}.${InjectedFunctions_1.default.markStatementForCompletion.name}("${fileName}",${l},${statementCounter})`);
            path.insertBefore(mark);
        }
        else {
            const enter = template_1.default.ast(`${ref}.${InjectedFunctions_1.default.enteringStatement.name}("${fileName}",${l},${statementCounter})`);
            path.insertBefore(enter);
            if (t.isReturnStatement(stmt)) {
                const rs = stmt;
                const call = t.callExpression(t.memberExpression(t.identifier(ref), t.identifier(InjectedFunctions_1.default.completingStatement.name)), [rs.argument, t.stringLiteral(fileName), t.numericLiteral(l), t.numericLiteral(statementCounter)]);
                path.replaceWith(t.returnStatement(call));
            }
            else {
                const completed = template_1.default.ast(`${ref}.${InjectedFunctions_1.default.completedStatement.name}("${fileName}",${l},${statementCounter})`);
        ...
        visitor: {
            ...
            Program: {
                enter(path, state) {
                    ...
                    objectives.length = 0;
                    //@ts-ignore
                    const srcFilePath = state.file.opts.filename;
                    //@ts-ignore
                    ...
                    const emImport = template_1.default.ast("const " + ref + " = require(\"evomaster-client-js\").InjectedFunctions;");
                    //@ts-ignore
                    path.unshiftContainer('body', emImport);
                    objectives.push(ObjectiveNaming_1.default.fileObjectiveName(fileName));
                },
                exit(path) {
                    //once the whole program is instrumented, the content of "objectives" array will be ready to be injected
                    const unique = Array.from(new Set(objectives)).sort();
                    const call = t.callExpression(t.memberExpression(t.identifier(ref), t.identifier(InjectedFunctions_1.default.registerTargets.name)), [t.arrayExpression(unique.map(e => t.stringLiteral(e)))]);
...
exports.default = evomasterPlugin;
//# sourceMappingURL=babel-plugin-evomaster.js.map